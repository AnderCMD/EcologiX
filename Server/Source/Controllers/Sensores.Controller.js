...
export const CrearSensor = async (req, res) => {
	try {
		...
		const { Nombre, Descripcion, Puerto, Velocidad_Transmision, Imagen } =
			req.body; // Obtener los datos del sensor
...
export const ActualizarSensor = async (req, res) => {
	try {
		...
		); // Buscar sensor por id y actualizarlo

		// Si no se encuentra el sensor
		if (!Sensor) {
			console.log('⚠️ ¡Sensor no encontrado!');
			return res.status(404).json({ message: '⚠️ Sensor no encontrado' }); // Responder con el mensaje de error
		} else {
