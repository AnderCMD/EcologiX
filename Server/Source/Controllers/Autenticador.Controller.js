...
export const Registro = async (req, res) => {
	...
	try {
        ...
        const Token = await CrearTokenAcceso({ ID_Usuario: UsuarioGuardado._id}); // Crear token de acceso
		
        console.log('✅ ¡Usuario registrado exitosamente!');
        
        res.cookie('Token', Token, { httpOnly: true }); // Enviar token al cliente
		res.json({
            ID_Usuario: UsuarioGuardado._id, // ID del usuario
...
export const Login = async (req, res) => {
	...
	try {
        ...
        if (!UsuarioEncontrado) {
            ...
            return res.status(400).json({ message: '⚠️ Correo o contraseña incorrectos' }); // Enviar respuesta al cliente
        }

        const PasswordCoincide = await bcrypt.compare(Password, UsuarioEncontrado.Password); // Comparar contraseñas
        ...
        const Token = await CrearTokenAcceso({ ID_Usuario: UsuarioEncontrado._id}); // Crear token de acceso
		
        console.log('✅ ¡Usuario logueado exitosamente!');
        
        res.cookie('Token', Token, { httpOnly: true }); // Enviar token al cliente
		res.json({
            ID_Usuario: UsuarioEncontrado._id, // ID del usuario
...
export const Logout = (req, res) => {
    try { // Intentar desloguear usuario
        res.cookie('Token', '', { 
            expires: new Date(0) 
        }); // Eliminar cookie
...
export const Perfil = async (req, res) => {
    const UsuarioEncontrado = await UsuarioModel.findById(req.Usuario.ID_Usuario); // Buscar usuario por ID
    
    if (!UsuarioEncontrado) return res.status(400).json({ message: '⚠️ Usuario no encontrado' }); // Si no se encuentra el usuario
